    ...
    if(repo == null) {
        ...
        if (!blobstat.repositories.containsKey(repo)) {
            if (!unmapped.contains(repo)) {
                ...
                // File Asset - Start
                def fname = properties.'@BlobStore.blob-name'
                def fsize = properties.'size'
                log.info("FILEASSET: " + ":" + fname + ":" + fsize)
                // File Asset - Finish
        ...
        if (blobstat.repositories.containsKey(repo)) {
            ...
            if (properties.'deleted') {
                ...
                if (!repo.equals('SYSTEM:direct-path')) {
                    ...
                    // Deleted Asset - Start
                    def frepo = properties.'@Bucket.repo-name'
                    def fname = properties.'@BlobStore.blob-name'
                    def fsize = properties.'size'
                    log.info("FILEASSET-DELETED: " + frepo + ":" + fname + ":" + fsize)
                    // Deleted Asset - Finish
